...
Patch release — Prettier-only fix.

### Fixed

- `tests/canonical-json.test.ts`: a `Set` literal in the cross-binding
  fixture-replay block was multi-line, while Prettier wanted it
  inline. The lint job on main rejected the multi-line form; the
  v0.5.0 publish chain wasted a slot waiting on a CI gate it could
  not pass. The fix changes whitespace only — no runtime behaviour
