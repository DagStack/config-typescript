...
| `_meta/secret_schemes.yaml` | `SecretScheme` string-enum (`ENV`, `VAULT`, `AWSSM`, `GCPSM`, `K8SSECRET`) + `SECRET_SCHEMES` registry of `SecretSchemeEntry(scheme, adapter, mandatory, phase, kind)` rows. Bindings use the enum to validate `${secret:<scheme>:...}` tokens at parse time, and the registry to surface adapter metadata in diagnostics. |
