...
- **Sort order** — keys sorted in UTF-16 code-unit order per RFC 8785 §3.2.3, not by locale. See `_meta/canonical_json.yaml` for per-binding comparator details (Python `sorted()` UTF-32, Go `sort.Strings()` UTF-8 bytes and JavaScript explicit byte comparators all diverge — pre-encode keys as UTF-16-BE bytes or apply an equivalent comparator).
