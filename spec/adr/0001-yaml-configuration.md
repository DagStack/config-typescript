...
- **Status:** accepted
- **Revision:** 2.3 (2026-05-19) — canonical-JSON sort-order correction
- **Date:** 2026-05-19 (v2.3), 2026-04-23 (v2.2, v2.1), 2026-04-19 (v2.0), 2026-04-17 (v1.0)
- **Architect review:** ai-systems-architect (v1.0 — single round 2026-04-17; v2.0 — must-fix round 2026-04-19; v2.1 — must-fix round 2026-04-23; v2.2 — pre-release hardening round 2026-04-23; v2.3 — RFC 8785 alignment round 2026-05-19)
- **Supersedes:** —
- **Revision history:**
  ...
  - v2.3 (2026-05-19) — RFC 8785 alignment in §9.1.1 and `_meta/canonical_json.yaml`:
    - **Spec text correction.** Previously the `keys.sort_order` field said "lexicographic UTF-8 code-point order" with a comment that this matches Python `sorted()`, JavaScript `.sort()`, and Go `sort.Strings()`. Two errors were corrected: (a) "UTF-8 code-point" is a category error (UTF-8 encodes Unicode code points as bytes — "code-point order" and "byte order" are distinct, agreeing only in the BMP); (b) the three native sort routines diverge — Python `sorted()` is UTF-32 code-point order, JavaScript `Array.prototype.sort` is UTF-16 code-unit order (ECMA-262 §22.1.3.27, conformant), Go `sort.Strings()` is UTF-8 byte order. RFC 8785 §3.2.3 mandates UTF-16 code-unit order.
    - **Cross-binding wire contract.** See `logger-spec` `_meta/fixtures/canonical_json_key_order.json` for the five-case normative fixture (drift-witness: BMP-PUA U+E000 vs CJK Ext B U+20000). A config-spec-local copy will land in a follow-up under `conformance/`.
    - **Binding conformance status at the time of this amendment.** All three v0.4 bindings are **not yet conformant**:
      - `config-python` v0.5.0 uses `json.dumps(..., sort_keys=True)` — Python's `sorted()` sorts by UTF-32 code-point order.
      - `config-typescript` v0.4.0 uses an explicit `compareUtf8Bytes` comparator — UTF-8 byte order (the binding deliberately overrides JavaScript's default UTF-16 code-unit sort to match Python/Go, an alignment that turned out to be wrong because Python/Go themselves diverge from RFC 8785).
      - `config-go` v0.4.0 uses `sort.Strings()` — UTF-8 byte order.

      All three converge on **non-RFC-8785** behaviour. The fix is the same in all bindings: sort by UTF-16 code-unit order — either by pre-encoding keys as UTF-16-BE bytes (Python, Go) or by relying on JavaScript's default `Array.prototype.sort` without an override (TypeScript). Coordinated binding bump required: tracked as `config-python` issue #25, `config-typescript` issue #24, and `config-go` issue #28.
    - **Public-registry clean-slate.** Bindings have not shipped to PyPI / npmjs.org / Go vanity. Any internal Nexus consumers run v0.4-v0.5 bindings against configs without supplementary-plane keys, so the divergent byte output has not been observed in practice; they will pick up the fix in the coordinated bump train.
    - **Spec → binding handoff acceptance.** This spec amendment is considered fully landed when (a) all three bindings tag a release that produces the expected `drift_witness_pua_vs_supplementary` wire bytes against the conformance fixture, and (b) the cross-binding roundtrip CI is green. Until then, spec is at v2.3 and bindings remain at "non-conformant" status.
    - **Related cross-binding amendment**: `logger-spec` parity epic #2 (same UTF-16 mandate, same fixture pattern; `logger-spec` ADR-0001 v1.3, PR #12).
    - Closes issue #29.
...
- Built on **RFC 8785 (JSON Canonicalization Scheme)** subset: object keys sorted in **UTF-16 code-unit order** (RFC 8785 §3.2.3 — see `_meta/canonical_json.yaml` for the per-binding comparator notes; native Python `sorted()` and Go `sort.Strings()` diverge), no whitespace except inside strings, integers without a decimal point (`1`), floats in ECMAScript shortest round-trip representation, NaN / Infinity forbidden.
